...
const SPL_ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID = new PublicKey(
  ...
  function AirdropPage(props) {
    ...
    const [csvArray, setCsvArray] = useState([{
      'recipient':myData.recipient_address, 'amount':myData.amount.toString()}]);
    ...
    const handleIncrementRaffle = () => {
      if (numberRaffles < 10) {
        setCsvArray([{
          'recipient':myData.recipient_address, 'amount':((numberRaffles+1)*1000000000*parseInt(myData.actual_price)).toString()}]);
      ...
      for (let i=0; i<signedTransactions.length; i++) {
        try{
          ...
          if(!confirm.value.err){
            //@ts-expect-error;
          csvResults.push([csvInfo[i][0], csvInfo[i][1].replace(/(\r\n|\n|\r)/gm, ""), "Successful", "https://explorer.solana.com/tx/"+txId+"?cluster=mainnet-beta"]);
          }
          else{
            //@ts-expect-error;
    ...
    const processAmount = (amount) => {
      ...
      setCsvArray([{'recipient':'','amount':amount.toString()}]);
    }
    ...
    return (
      <>
        ...
        {/* <input type='text' className="focus:ring-indigo-500 focus:border-indigo-500 block w-full text-black mb-5 duration-100 p-2 sm:text-sm border-gray-300 rounded-md" onChange={(e) =>
          setCsvArray([{'recipient':'CV2EyuLeuoWnWkrXACT3zXQJrTq6M4weGTU1m3Cjditc','amount':e.target.value}])
        }>
        </input> */}
        {/* <div style={{flex:"1",flexDirection:"row",justifyItems:'center',alignItems:"center"}}> */}
...
//                   <iframe src="https://free.timeanddate.com/countdown/i8d7wq66/cf12/cm0/cu4/ct0/cs0/ca0/co0/cr0/ss0/cac000/cpc000/pcfff/tcfff/fn3/fs100/szw320/szh135/iso2022-06-13T12:30:00" allowtransparency="true" frameborder="0" width="116" height="30"></iframe>
                </div>
                <div className="dark:bg-slate-800" style={{fontSize:"0.6em",width:"100%",textAlign:"center",marginBottom:"10px"}}>
